...
sys.path.append('/home/lab302/Desktop/tsx/Efficient-Segmentation')
matplotlib.use('Agg')
...
def parse_args():
    ...
    # model and dataset
    parser.add_argument('--model', type=str, default="FFENet", help="model name: (default ENet)")
    ...
    parser.add_argument('--classes', type=int, default=11,
                        help="the number of classSpecifies a target value that is ignored and does not contribute to the input gradientes in the dataset. 19 and 11 for cityscapes and camvid, respectively")
    parser.add_argument('--train_type', type=str, default="trainval",
                        help="ontrain forzzz training on train set, ontrainval for training on train+val set")
    # training hyper paramscityscapes
    ...
    # checkpoint and log
    parser.add_argument('--resume', type=str, default="checkpoint/cityscapes/FFENetbs6gpu1_trainval/model_350.pth",
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        output = model(images)
        if args.model != 'FFENet':
            loss = criterion(output, labels)
        else:
            aux_loss0 = criterion(output[0], labels)
            aux_loss1 = criterion(output[1], labels)
            main_loss = criterion(output[2], labels)
