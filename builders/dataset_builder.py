...
def build_dataset_train(dataset, input_size, batch_size, train_type, random_scale, random_mirror, num_workers):
    ...
    if not os.path.isfile(inform_data_file):
        ...
        datas = dataCollect.collectDataAndSave()#inform_data_file collect the information of mean, std and weigth_class
        if datas is None:
    ...
    if dataset == "cityscapes":
        ...
        valLoader = data.DataLoader(
            ...
            batch_size=1, shuffle=True, num_workers=num_workers, pin_memory=False,
            drop_last=True)
        ...
        # 这样将内存的Tensor转义到GPU的显存就会更快一些,默认为fasle，计算机的内存充足的时候，可以设置pin_memory=True。
