...
class CityscapesTrainInform:
    ...
    def readWholeTrainSet(self, fileName, train_flag=True):
        ...
        self.compute_class_weights(global_hist)#原理暂未找到
        return 0
...
if __name__ == '__main__':
    ...
    train_data_list = os.path.join(data_dir, dataset + '_' + 'trainval' + '_list.txt')
    cd =  CityscapesDataSet(data_dir,train_data_list)
    trainLoader = data.DataLoader(cd)
    for step, (batch_x, batch_y,_,_) in enumerate(trainLoader):
        pass
        # training
