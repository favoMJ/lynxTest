...
class CrossEntropyLoss2dLabelSmooth(_WeightedLoss):
    ...
    def forward(self, output, target):
        ...
        targets = (1 - self.epsilon) * targets + self.epsilon / n_classes#先进行label_smoothing 再使用CrossEntropyLoss
...
class ProbOhemCrossEntropy2d(nn.Module):
    ...
    def forward(self, pred, target):
        b, c, h, w = pred.size()
        target = target.view(-1)
        valid_mask = target.ne(self.ignore_label)#逐个比较 ， 不符合的返回1
        ...
        if self.min_kept > num_valid:
            ...
            if self.min_kept > 0:
                ...
                if mask_prob[threshold_index] > self.thresh:
                    threshold = mask_prob[threshold_index]#保持数目后的threshold
                kept_mask = mask_prob.le(threshold)#input < thrreshold
                target = target * kept_mask.long()
                valid_mask = valid_mask * kept_mask
                # print('Valid Mask: {}'.format(valid_mask.sum()))
        #是将 mask中为1的 元素所在的索引，在a中相同的的索引处替换为 value  ,mask value必须同为tensor
        target = target.masked_fill_(~ valid_mask, self.ignore_label)#筛选后
